...
class Keypool(object):
    ...
    def keypool_update(self, keypool, tenant, versioning, apitype, uid, secret):
        '''
	...
	try:
	    if(apitype == 's3'):
		return s3_bucket_update(namespace, keypool, versioning , uid, secretkey )
	    else:
        	raise SOSError(SOSError.VALUE_ERR, 
                       "Wrong API type " + apitype + " specified")
    ...
    def keypool_delete(self, keypool, tenant, uid, secret):
        '''
        Makes a REST API call to delete a keypool by its name, project and tenant 
        '''
	...
	try:
		...
		return swift_container_delete(namespace, keypool, uid, secretkey )
	    elif(apitype == 'atmos'):
		return atmos_subtenant_delete(namespace, keypool, uid, secretkey )
	    else:
        	raise SOSError(SOSError.VALUE_ERR, 
                       "Wrong API type " + apitype + " specified")
	except SOSError as e:
	...
	try:
		...
		kplst.append(s3_bucket_list(namespace,projectname, uid, secretkey ))
	    if((not apitype) or (apitype == 'swift')):
		kplst.append(swift_container_list(namespace, projectname, uid, secretkey ))
	    if((not apitype) or (apitype == 'atmos')):
		kplst.append(atmos_subtenant_list(namespace, projectname, uid, secretkey ))
	...
	(s, h) = common.service_json_request(self.__ipAddr, ATMOS_PORT , "PUT",
                                             self.URI_SWIFT_CONTAINER_INSTANCE.format(namespace, container) , None, None, False , 
	...
	(s, h) = common.service_json_request(self.__ipAddr, ATMOS_PORT , "DELETE",
                                             URI_ATMOS_SUBTENANT_INSTANCE.format(subtenant) , None, None, False , 
					     'application/json', None, _headers)
	...
	(s, h) = common.service_json_request(self.__ipAddr, SWIFT_PORT , "DELETE",
                                             URI_SWIFT_CONTAINER_INSTANCE.format(namespace, container) , None, None, False , 
					     'application/json', None, _headers)
	

    def swift_container_show(namespace, container, uid, secret):
	...
	(s, h) = common.service_json_request(self.__ipAddr, SWIFT_PORT , "GET",
                                             URI_SWIFT_ACCOUNT_INSTANCE.format(namespace), None, None, False , 
					     'application/json', None, _headers)
	...
	(s, h) = common.service_json_request(self.__ipAddr, SWIFT_PORT , "GET",
                                             URI_S3_BUCKET_INSTANCE.format(keypool) , None, None, False , 
	...
	_headers = common.s3_hmac_base64_sig('PUT', bucket, None , uid , secret, 'application/json' , _headers, None )
    ...
    def s3_bucket_update(namespace, bucket, versioning, uid, secret):
	headers = dict()
	...
	_headers = common.s3_hmac_base64_sig('PUT', bucket, None , uid, secret, 'application/json' , _headers, qparms )
	...
	_headers = common.s3_hmac_base64_sig('GET', bucket, None , uid , secret, 'application/json' , _headers, None )
	...
	_headers = common.s3_hmac_base64_sig('DELETE', bucket, None , uid, secret, 'application/json' , _headers, None )
...
def create_parser(subcommand_parsers, common_parser):
    ...
    create_parser = subcommand_parsers.add_parser('create',
                                description='SOS Keypool Create CLI usage.',
...
def delete_parser(subcommand_parsers, common_parser):
    ...
    delete_parser = subcommand_parsers.add_parser('delete',
                                description='SOS Keypool delete CLI usage.',
                                parents=[common_parser],
                                conflict_handler='resolve',
...
def show_parser(subcommand_parsers, common_parser):
    ...
    show_parser = subcommand_parsers.add_parser('show',
                                description='SOS Keypool Show CLI usage.',
                                parents=[common_parser],
                                conflict_handler='resolve',
                                help='Show a Keypool')
...
def list_parser(subcommand_parsers, common_parser):
    ...
    list_parser = subcommand_parsers.add_parser('list',
                                                description='SOS Keypool List CLI usage.',
                                                parents=[common_parser],
                                                conflict_handler='resolve',
...
def keypool_list(args):
    obj = Keypool(args.ip, args.port)
    try:
        lst = obj.keypool_list(args.project, args.tenant, args.apitype, args.uid, args.secret)
...
def update_parser(subcommand_parsers, common_parser):
    ...
    update_parser = subcommand_parsers.add_parser('update',
                                description='SOS Keypool update CLI usage.',
                                parents=[common_parser],
                                conflict_handler='resolve',
...
def keypool_update(args):
    obj = Keypool(args.ip, args.port)
    try:
        res = obj.keypool_update(args.name, args.tenant, args.versioning, args.apitype, args.uid, args.secret)
    except SOSError as e:
        if (e.err_code in [SOSError.NOT_FOUND_ERR]):
            raise SOSError(e.err_code, "Keypool " + 
