...
class ConsistencyGroup(object):
    ...
    def create(self, name, project, tenant):
        '''
        This function will take consistency group name and project name  as input and
        It will create the consistency  group with given name.
        parameters:
           name : Name of the consistency group.
           project: Name of the project path.
    ...
    def update(self, name, project, tenant, add_volumes, remove_volumes):
        '''
    ...
    def delete(self, name, project, tenant):
        '''
        This function will take consistency group name and project name as input and 
        marks the particular consistency group as delete.
        parameters:
           name : Name of the consistency group.
           project: Name of the project.
        return
    ...
    def consistencygroup_query(self, name, project, tenant):
        '''
        This function will take consistency group name/id and project name  as input and 
        returns consistency group id.
        parameters:
        ...
        for uri in uris:
            ...
            if(congroup):
                if (congroup['name'] == name ):
                    return congroup['id']    
        raise SOSError(SOSError.NOT_FOUND_ERR, "Consistency Group " + name + ": not found")
    
    def snapshot_create(self, name, project, tenant, snapshotname, createinactive):
        '''
        ...
        (s, h) = common.service_json_request(self.__ipAddr, self.__port,
                        "POST",
                        self.URI_CONSISTENCY_GROUPS_SNAPSHOT.format(uri),
                        body, None)
        o = common.json_decode(s)
        return o
    ...
    def snapshot_list(self, name, project, tenant):
        '''
        This function will list all the snapshot in the given consistency group.
        return the status of the command.
        parameters:
             name : name of the consistency group
             project: name of the project name
    ...
    def snapshot_show(self, name, project, tenant, snapshotname):
        '''
        This function will show the details of given snapshot under a consistency group.
    ...
    def snapshot_query(self, name, project, tenant, snapshotname):
        ...
        for ss in uris:
            if (ss['name'] == name):
                return ss['id']
        raise SOSError(SOSError.SOS_FAILURE_ERR, "Snapshot " + name +
                       ": not found")
...
def create_parser(subcommand_parsers, common_parser):
    ...
    create_parser = subcommand_parsers.add_parser('create',
                description='StorageOS Consistency group Create cli usage.',
...
def update_parser(subcommand_parsers, common_parser):
    ...
    update_parser = subcommand_parsers.add_parser('update',
                description='StorageOS Consistency group Update cli usage.',
                parents=[common_parser],
                conflict_handler='resolve',
    ...
    update_parser.add_argument('-remove_volumes', '-rv',
...
def consistencygroup_update(args):
    try:
        ...
        res = obj.update(args.name, args.project, args.tenant, 
                         args.add_volumes, args.remove_volumes)
    except SOSError as e:
        raise SOSError(SOSError.SOS_FAILURE_ERR, "Consistency Group " + args.name + ": Update failed:\n" + e.err_text)
...
def delete_parser(subcommand_parsers, common_parser):
    ...
    delete_parser = subcommand_parsers.add_parser('delete',
                description='StorageOS Consistency group Delete CLI usage.',
                parents=[common_parser],
                conflict_handler='resolve',
...
def consistencygroup_delete(args):
    ...
    try:
        ...
        raise SOSError(SOSError.SOS_FAILURE_ERR, "Consistency Group " + args.name + ": Delete failed:\n" + e.err_text)
...
def list_parser(subcommand_parsers, common_parser):
    ...
    list_parser = subcommand_parsers.add_parser('list',
                description='StorageOS Consistency group List CLI usage.',
                parents=[common_parser],
                conflict_handler='resolve',
...
def show_parser(subcommand_parsers, common_parser):
    ...
    show_parser = subcommand_parsers.add_parser('show',
                description='StorageOS Consistency group Show CLI usage.',
                parents=[common_parser],
                conflict_handler='resolve',
...
def snapshot_parser(subcommand_parsers, common_parser):
    snapshot_parser = subcommand_parsers.add_parser('snapshot',
                                parents=[common_parser],
                                conflict_handler='resolve',
                                help='Snapshot operation through consistency group')
    subcommand_parsers = snapshot_parser.add_subparsers(help='Use one of the commands')

    #snapshot create 
    sscreate_parser=subcommand_parsers.add_parser('create',
                                parents=[common_parser],
                                conflict_handler='resolve',
    ...
    #snapshot list
    sslist_parser=subcommand_parsers.add_parser('list',
                                parents=[common_parser],
                                conflict_handler='resolve',
    ...
    #snapshot show
    ssshow_parser=subcommand_parsers.add_parser('show',
                                parents=[common_parser],
                                conflict_handler='resolve',
    ...
    mandatory_args.add_argument('-snapshotname', '-ssn',
                                ...
                                required=True)
    #snapshot activate
    ssactivate_parser=subcommand_parsers.add_parser('activate',
                                parents=[common_parser],
                                conflict_handler='resolve',
                                help='activate snapshot in consistency group')
    mandatory_args = ssactivate_parser.add_argument_group('mandatory arguments')
    mandatory_args.add_argument('-name', '-n',
                                metavar='<consistencygroupname>',
                                dest='name',
                                help='Name of Consistency Group',
    ...
    #snapshot deactivate
    ssdeactivate_parser=subcommand_parsers.add_parser('deactivate',
                                parents=[common_parser],
                                conflict_handler='resolve',
                                help='deactivatee snapshot in consistency group')
    mandatory_args = ssdeactivate_parser.add_argument_group('mandatory arguments')
    mandatory_args.add_argument('-name', '-n',
                                metavar='<consistencygroupname>',
                                dest='name',
                                help='Name of Consistency Group',
    ...
    #snapshot restore
    ssrestore_parser=subcommand_parsers.add_parser('restore',
                                parents=[common_parser],
                                conflict_handler='resolve',
...
def consistencygroup_parser(parent_subparser, common_parser):
    ...
    parser = parent_subparser.add_parser('consistencygroup',
                description='StorageOS Consistency Group cli usage.',
                parents=[common_parser],
                conflict_handler='resolve',
