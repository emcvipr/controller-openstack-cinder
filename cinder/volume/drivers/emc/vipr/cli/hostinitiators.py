...
class HostInitiator(object):
    ...
    def query_by_name(self, initiatorName, hostName):
        ...
        raise SOSError(SOSError.NOT_FOUND_ERR, "Initiator with name '" +initiatorName+ "' not found")
    ...
    def query_by_portwwn(self, initiatorWWN, hostName):
        ...
        for initiator in initiatorList:
            ...
            if(initiatorDetails is not None and initiatorDetails['initiator_port'] == initiatorWWN):
                return initiator['id']
        
        raise SOSError(SOSError.NOT_FOUND_ERR, "Initiator with WWN '" +initiatorWWN+ "' not found")
        
    
    """
    Initiator create operation
    """
    def create(self, hostlabel, protocol, initiatorwwn, portwwn):
    ...
    def list_all(self):
        ...
        if(hostUris.__len__() > 0):
            ...
            raise SOSError(SOSError.NOT_FOUND_ERR, "No initiators found in the system")
    ...
    def show_by_protocol(self, initiatorList, protocol):        
        ...
        if(initiatorList is not None):
            for initiator in initiatorList:
                ...
                if(initiatorDetail is not None and len(initiatorDetail)>0 and initiatorDetail['protocol'] == protocol):
            ...
            for initiator in initiatorList:
                ...
                if(initiatorDetail is not None and len(initiatorDetail)>0 
                   and initiatorDetail['name'] == name):
                    if(xml):
                        initiatorListDetails = self.show_by_uri(initiatorUri, xml)
                    else:
...
def initiator_create(args):
    ...
    if(args.protocol == "FC" and args.initiatorwwn is None):
        raise SOSError(SOSError.CMD_LINE_ERR, sys.argv[0] + " " + sys.argv[1] + 
                           " " + sys.argv[2] + ": error:"+ "-inititorwwn is required for FC type initiator")
    
    if(args.protocol == "iSCSI" and args.initiatorwwn):
        raise SOSError(SOSError.CMD_LINE_ERR, sys.argv[0] + " " + sys.argv[1] + 
                           " " + sys.argv[2] + ": error:"+ "-inititorwwn is not required for iSCSI type initiator")
        
    
        
    initiatorObj = HostInitiator(args.ip, args.port)
    try:
        initiatorObj.create(args.hostlabel, args.protocol, args.initiatorwwn, args.initiatorportwwn)
    except SOSError as e:
        common.format_err_msg_and_raise("create", "initiator", e.err_text, e.err_code)
...
def initiator_delete(args):
    try:
        ...
        common.format_err_msg_and_raise("delete", "initiator", e.err_text, e.err_code)
...
def update_parser(subcommand_parsers, common_parser):
    ...
    update_parser.add_argument('-nwwn', '-newinitiatorwwn',
                                help='WWN of the initiator node ',
                                dest='newinitiatorwwn',
                                metavar='<newinitiatorwwn>')
    
    mandatory_args.add_argument('-npwwn', '-newinitiatorportwwn',
                                help='Port wwn, it can be WWN for FC, IQN/EUI for iSCSI',
...
def initiator_update(args):
    ...
    if(args.newprotocol is None and args.newinitiatorwwn is None and args.newinitiatorportwwn is None):
        raise SOSError(SOSError.CMD_LINE_ERR, sys.argv[0] + " " + sys.argv[1] + 
    ...
    if(args.newprotocol == "iSCSI" and args.newinitiatorwwn):
        raise SOSError(SOSError.CMD_LINE_ERR, sys.argv[0] + " " + sys.argv[1] + 
                           " " + sys.argv[2] + ": error:"+ "-newinititorwwn is not required for iSCSI type initiator")
    
    initiatorObj = HostInitiator(args.ip, args.port)
    try:
        
        initiatorUri = initiatorObj.query_by_portwwn(args.initiatorportwwn, args.hostlabel)
        initiatorObj.update(initiatorUri, args.newprotocol, args.newinitiatorwwn, args.newinitiatorportwwn)
        
    except SOSError as e:
        common.format_err_msg_and_raise("update", "initiator", e.err_text, e.err_code)      
...
def initiator_parser(parent_subparser, common_parser):
    ...
    parser = parent_subparser.add_parser('initiator',
