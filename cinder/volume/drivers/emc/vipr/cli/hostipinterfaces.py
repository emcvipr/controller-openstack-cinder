...
class HostIPInterface(object):
    ...
    def query_by_name(self, ipinterfaceName):
        ...
        raise SOSError(SOSError.NOT_FOUND_ERR, "Ip-interface with name '" +ipinterfaceName+ "' not found")
    ...
    def query_by_ipaddress(self, ipInterfaceIp, hostName):
        ...
        for ipinterface in ipinterfaceList:
            ...
            if(ipinterfaceDetails is not None and ipinterfaceDetails['ip_address'] == ipInterfaceIp):
                return ipinterface['id']
        
        raise SOSError(SOSError.NOT_FOUND_ERR, "Ip-interface with IP-Address '" +ipInterfaceIp+ "' not found")
        
    
    """
    ipinterface create operation
    """
    def create(self, hostlabel, protocol, ipAddress, netMask, prefixLength, scopeId):
    ...
    """
    def update(self, hostName, ipinterfaceAddress, newprotocol, newipAddress, newNetMask, newPrefixLength, newScopeId):
    ...
    def list_all(self):
        ...
        if(hostUris.__len__() > 0):
            ...
            raise SOSError(SOSError.NOT_FOUND_ERR, "No ip-interfaces found in the system")
    ...
    def list_all_by_uri(self):
        ...
        if(hostUris.__len__() > 0):
            ...
            raise SOSError(SOSError.NOT_FOUND_ERR, "No hosts found in the system")
    ...
    def show_by_protocol(self, ipinterfaceList, protocol):        
        ...
        if(ipinterfaceList is not None):
            for ipinterface in ipinterfaceList:
                ...
                if(ipinterfaceDetail is not None and len(ipinterfaceDetail)>0 and ipinterfaceDetail['protocol'] == protocol):
            ...
            for ipinterface in ipinterfaceList:
                ...
                if(ipinterfaceDetail is not None and len(ipinterfaceDetail)>0 
                   and ipinterfaceDetail['name'] == name):
                    if(xml):
                        ipinterfaceListDetails = self.show_by_uri(ipinterfaceUri, xml)
                    else:
...
def create_parser(subcommand_parsers, common_parser):
    ...
    mandatory_args.add_argument('-hl', '-hostlabel',
                                ...
                                dest='hostlabel',
                                required=True)
    mandatory_args.add_argument('-pt', '-protocol ',
    ...
    create_parser.add_argument('-pl', '-prefixlength',
...
def ipinterface_create(args):
    ...
    try:
        ipinterfaceObj.create(args.hostlabel, args.protocol, args.ipaddress, args.netmask, args.prefixlength, args.scopeid)
    except SOSError as e:
        common.format_err_msg_and_raise("create", "ip-interface", e.err_text, e.err_code)
...
def list_parser(subcommand_parsers, common_parser):
    ...
    mandatory_args.add_argument('-hl', '-hostlabel',
                               ...
                               required=True)
    list_parser.add_argument('-pt', '-protocol ',
                               choices=HostIPInterface.IPINTERFACE_PROTOCOL_LIST,
                               dest='protocol',
                               help='ip-interface protocol')
...
def ipinterface_list(args):
    ...
    try:
        if(args.hostlabel):
            ...
            ipinterfaceList = ipinterfaceObj.get_host_object().list_ipinterfaces(hostUri)
        else:
        ...
        if(len(ipinterfaceList)>0):
            ...
            if(args.protocol is None):
                ...
                ipinterfaceListDetails = ipinterfaceObj.show_by_protocol(ipinterfaceList, args.protocol)
            
            if(args.verbose == True):
                return common.format_json_object(ipinterfaceListDetails)
            else:
                if(args.largetable == True):
                    TableGenerator(ipinterfaceListDetails, ['name', 'ip_address', 'protocol','netmask', 'prefix_length']).printTable()
                else:
                    TableGenerator(ipinterfaceListDetails, ['name', 'ip_address', 'protocol']).printTable()
       
        
    except SOSError as e:
        common.format_err_msg_and_raise("list", "ip-interface", e.err_text, e.err_code)
...
def ipinterface_show(args):
    ...
    try:
        ipinterfaceList = []
        ipinterfaceObj = HostIPInterface(args.ip, args.port)        
        interfaceUri = ipinterfaceObj.query_by_ipaddress(args.ipaddress, args.hostlabel)    
        ...
        common.format_err_msg_and_raise("show", "ip-interface", e.err_text, e.err_code)
...
def ipinterface_delete(args):
    try:
        ...
        common.format_err_msg_and_raise("delete", "ip-interface", e.err_text, e.err_code)
...
def update_parser(subcommand_parsers, common_parser):
    ...
    mandatory_args.add_argument('-hl', '-hostlabel',
                               ...
                               help='Host for which ip-interfaces to be searched',
                               required=True)
    update_parser.add_argument('-npt', '-newprotocol ',
                               choices=HostIPInterface.IPINTERFACE_PROTOCOL_LIST,
                               dest='newprotocol',
    ...
    update_parser.add_argument('-npl', '-newprefixlength',
...
def ipinterface_update(args):
    ...
    if(args.newprotocol is None and args.newipaddress is None and args.newnetmask is None
       and args.newprefixlength is None and args.newscopeid):
    ...
    ipinterfaceObj = HostIPInterface(args.ip, args.port)
    try:
        ipinterfaceObj.update(args.hostlabel, args.ipaddress, args.newprotocol, args.newipaddress, args.newnetmask, args.newprefixlength, args.newscopeid)
    except SOSError as e:
        common.format_err_msg_and_raise("update", "ipinterface", e.err_text, e.err_code)      
...
def ipinterface_parser(parent_subparser, common_parser):
    ...
    parser = parent_subparser.add_parser('ipinterface',
