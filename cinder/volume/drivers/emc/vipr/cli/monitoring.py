...
class Monitor(object):
    ...
    def get_events(self, output_format, time_frame):
        ...
        (s, h) = common.service_json_request(self.__ipAddr, self.__port,
                                            "GET",
                                            Monitor.URI_MONITOR.format(output_format, time_frame),
                                            None)
...
def monitor_get_events(args):
    ...
    try:
        if( int(args.year) <=  1900):
            print "error: year=" + args.year + " is before 1900, it require year >= 1900"
            return
...
def monitor_parser(parent_subparser, common_parser):
    ...
    parser = parent_subparser.add_parser('monitor',
                                description='SOS monitoring CLI usage',
                                parents=[common_parser],
                                conflict_handler='resolve',
                                help='Get monitoring events for the given time bucket')
