...
class StoragePool(object):
    ...
    def storagepool_list_by_uri(self, uri):
        (s, h) = common.service_json_request(self.__ipAddr, self.__port ,"GET", 
                         StoragePool.URI_STORAGEPOOLS.format(uri), None)
    ...
    def storagepool_list(self, devicename, devicetype, serialnumber=None):
        ...
        device_id = self.storagesystem_query(devicename, devicetype, serialnumber)
    ...
    def storagepool_show_by_uri(self, systemuri, pooluri, xml=False):
        ...
        '''
        (s, h) = common.service_json_request(self.__ipAddr, self.__port, "GET", 
                        StoragePool.URI_STORAGEPOOL_SHOW.format(systemuri, pooluri), None, None, xml)
    ...
    def storagepool_create(self, storagename, poolname,
                           ...
                           freeCapacity, totalCapacity, extensions, deviceType):
	...
	sstype = None
  	...
  	if (not common.is_uri(storagename)):
        ...
        if( (sstype == 'vnxblock') or (sstype == 'vnxfile') or (sstype == 'vmax') ):
    ...
    def storagepool_register(self, systemuri, pooluri):
        ...
        (s, h) = common.service_json_request(self.__ipAddr, self.__port, "POST", 
                        StoragePool.URI_STORAGEPOOL_REGISTER.format(systemuri,pooluri), body)
    ...
    def storagepool_delete(self, qualifiedname, storagesystem, serialnumber, devicetype):
	
        (device_id, pooluri) = self.storagepool_query(qualifiedname, storagesystem, serialnumber, devicetype)
    ...
    def storagepool_deregister(self, qualifiedname, storagesystem, serialnumber, devicetype):
	
        (device_id, pool_id) = self.storagepool_query(qualifiedname, storagesystem, serialnumber, devicetype)
        return self.storagepool_deregister_by_uri(device_id, pool_id)

    def storagepool_query(self, qualifiedname, storagesystem, serialnumber, devicetype):
	 ...
	 #storagesystem_query(self, devicename, devicetype, serialnumber=None):
    	except SOSError as e:
            raise e
    ...
    def storagepool_show(self,qualifiedname, storagesystem, serialnumber, devicetype, xml=False):
        (device_id, pooluri) = self.storagepool_query(qualifiedname, storagesystem, serialnumber, devicetype)
...
def create_parser(subcommand_parsers, common_parser):
    ...
    create_parser = subcommand_parsers.add_parser('create',
                                description='StorageOS Storagepool Create CLI usage.',
...
def storagepool_create(args):
    ...
    try:
        ...
        if( args.filesystemtype):
            extensions.append(STORAGEPOOL_EXTENSION.format('FileSystemType' , args.filesystemtype))
        if( args.thinprovisioned):
            extensions.append(STORAGEPOOL_EXTENSION.format('ThinProvisioned' , args.thinprovisioned))
        if( args.autoextendenabled):
            extensions.append(STORAGEPOOL_EXTENSION.format('AutoExtendEnabled' , args.autoextendenabled))
        if( args.pooltype):
            extensions.append(STORAGEPOOL_EXTENSION.format('PoolType' , args.pooltype))
        if( args.nativeid):
            extensions.append(STORAGEPOOL_EXTENSION.format('NativeId' , args.nativeid))
        if( args.controllerpoolname):
            extensions.append(STORAGEPOOL_EXTENSION.format('Name' , args.controllerpoolname))


        res = obj.storagepool_create(args.storagesystem, args.name, args.protocol, 
...
def register_parser(subcommand_parsers, common_parser):
    ...
    register_parser = subcommand_parsers.add_parser('register',
                                description='StorageOS Storagepool Register CLI usage.',
                                parents=[common_parser],
                                conflict_handler='resolve')
...
def delete_parser(subcommand_parsers, common_parser):
    ...
    delete_parser = subcommand_parsers.add_parser('delete',
                                description='StorageOS Storagepool Delete CLI usage.',
                                parents=[common_parser],
                                conflict_handler='resolve')
...
def storagepool_delete(args):
    obj = StoragePool(args.ip, args.port)
    try:
        res = obj.storagepool_delete(args.name, args.storagesystem, args.serialnumber, args.type)
    except SOSError as e:
        if( e.err_code == SOSError.NOT_FOUND_ERR):
            raise SOSError(SOSError.NOT_FOUND_ERR, 
...
def deregister_parser(subcommand_parsers, common_parser):
    ...
    deregister_parser = subcommand_parsers.add_parser('deregister',
                                description='StorageOS Storagepool Deregister CLI usage.',
                                parents=[common_parser],
                                conflict_handler='resolve')
...
def storagepool_deregister(args):
    obj = StoragePool(args.ip, args.port)
    try:
        res = obj.storagepool_deregister(args.name, args.storagesystem, args.serialnumber, args.type)
    except SOSError as e:
        if( e.err_code == SOSError.NOT_FOUND_ERR):
            raise SOSError(SOSError.NOT_FOUND_ERR, 
...
def show_parser(subcommand_parsers, common_parser):
    ...
    show_parser = subcommand_parsers.add_parser('show',
                                description='StorageOS Storagepool Show CLI usage.',
                                parents=[common_parser],
                                conflict_handler='resolve')
...
def storagepool_show(args):
    obj = StoragePool(args.ip, args.port)
    try:
        res = obj.storagepool_show(args.name, args.storagesystem, args.serialnumber,  args.type, args.xml)
...
def query_parser(subcommand_parsers, common_parser):
    ...
    query_parser = subcommand_parsers.add_parser('query',
                                description='StorageOS Storagepool Query CLI usage.',
                                parents=[common_parser],
                                conflict_handler='resolve')
...
def list_parser(subcommand_parsers, common_parser):
    ...
    list_parser = subcommand_parsers.add_parser('list',
                                description='StorageOS Storagepool List CLI usage.',
                                parents=[common_parser],
                                conflict_handler='resolve')
...
def storagepool_parser(parent_subparser, common_parser):
    ...
    parser = parent_subparser.add_parser('storagepool',
                description='StorageOS Storagepool CLI usage',
                parents=[common_parser],
                conflict_handler='resolve',
